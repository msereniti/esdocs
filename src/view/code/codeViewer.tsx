...
for (const language in programmingLanguages) {
  const languageDefinition = programmingLanguages[language as keyof typeof programmingLanguages].default;
...
export const CodeViewer: React.FC<{ language: string; sourceCode: string }> = ({ language, sourceCode }) => {
  ...
  return (
    <SyntaxHighlighter className="es-docs__code-viewer" language={language} showLineNumbers={false} style={style} wrapLongLines>
      {sourceCode}
    </SyntaxHighlighter>
  );
};

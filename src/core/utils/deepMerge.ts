...
type Zip_DeepMergeTwoTypes<T, U> = T extends [] ? U : U extends [] ? T : [DeepMergeTwoTypes<Head<T>, Head<U>>, ...Zip_DeepMergeTwoTypes<Tail<T>, Tail<U>>];
...
type MergeTwoObjects<
  ...
  T0 = Partial<GetObjDifferentKeys<T, U>> & { [K in keyof GetObjSameKeys<T, U>]: DeepMergeTwoTypes<T[K], U[K]> }, // shared keys are recursively resolved by `DeepMergeTwoTypes<...>`
...
export const mergeDeep = <A, B>(target: A, source: B): DeepMergeTwoTypes<A, B> => {
  ...
  return target as DeepMergeTwoTypes<A, B>;
};

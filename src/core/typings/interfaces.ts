...
export const serializeInterfaceDeclaration = (interfaceDeclaration: ts.InterfaceDeclaration): SerializedType => {
  ...
  interfaceDeclaration.forEachChild((child) => {
    ...
    if (child.kind === ts.SyntaxKind.TypeParameter && (child as ts.TypeParameterDeclaration).constraint) {
  ...
  const serializedProperties = properties.map((property) => serializeProperty(property, genericsMap));
